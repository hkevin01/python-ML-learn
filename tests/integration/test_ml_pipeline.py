...
from src.ml_core.supervised import evaluate_classification
from src.ml_core.supervised import evaluate_regression as eval_reg
from src.ml_core.unsupervised import evaluate_clustering, find_optimal_clusters
from src.utils.numpy_helpers import normalize, set_seed, standardize
from src.utils.pandas_helpers import detect_outliers_iqr, get_missing_info
...
class TestDataProcessingPipeline:
    ...
    def test_complete_preprocessing_flow(self, sample_dataset):
        ...
        # Step 4: Standardize (axis=0 means per-column)
        X_standardized = standardize(X, axis=0)
        # Mean should be close to 0 (with some tolerance due to float precision)
...
class TestRegressionPipeline:
    ...
    def test_complete_regression_flow(self, regression_data):
        ...
        np.testing.assert_almost_equal(results["rmse"], np.sqrt(results["mse"]))
...
class TestClusteringPipeline:
    ...
    def cluster_data(self):
        ...
        X, true_labels = make_blobs(
            n_samples=200, n_features=5, centers=3, cluster_std=1.0, random_state=42
        )
